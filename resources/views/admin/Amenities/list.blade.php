...
@section('title')
    <title>@lang('text.AmenitiesTitle')</title>
@endsection
@section('main')
        ...
        <div class="row">
            @can('add-amenities')
                <div class="col-12 mb-5">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title {{app()->getLocale() != "fa" ? "" : "text-right"}}">@lang('text.AmenitiesTitleAdd')</h4>
            ...
            <div class="col-12 d-none d-lg-block">
                <table class="table table-striped">
                    ...
                    <tbody>
                    @foreach($data['amenities'] as $key => $item)
                        <tr class="text-center">
                            ...
                            @can('edit-amenities')
                                <td>
                                    <a class="btn-primary btn"
                                       href="{{route('amenities.edit',['amenity'=>$item->id])}}"><i
                                            class="fa fa-pencil"></i></a>
                                </td>
                            @endcan
                            @can('delete-amenities')
                                <td>
                                    <form action="{{route('amenities.destroy',['amenity'=>$item->id])}}" method="POST">
                                        @csrf
                                        @method('delete')
                    ...
                    <ul style="display: none">
                        ...
                        @can('delete-amenities')
                            <li>
                                ...
                                <span>
                                        <form action="{{route('amenities.destroy',['amenity'=>$item->id])}}"
                                              method="POST">
                                        @csrf
                                            @method('delete')
                                        <button class="btn-danger btn delete-submit" hidden><i class="fa fa-trash"></i>
