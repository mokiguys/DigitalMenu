        ...
        <div class="row">
            @can('add-province')
                <div class="col-12 mb-5">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title {{app()->getLocale() != "fa" ? "" : "text-right"}}">@lang('text.ProvinceTitleAdd')</h4>
                            <form class="forms-sample" method="post" action="{{route('province.store')}}"
                ...
                <table class="table table-striped">
                    ...
                    @foreach($data['province'] as $key => $item)
                        <tr class="text-center">
                            ...
                            <td>{{$item->en}}</td>
                            <td>{{$item->tr}}</td>
                            <td>{{$item->country_id == 2 ? $data['country'][0][app()->getLocale()] : $data['country'][1][app()->getLocale()]}}</td>
                            @can('edit-province')
                                <td>
                                    <a class="btn-primary btn"
                                       href="{{route('province.edit',['province' => $item->id])}}"><i
                                            class="fa fa-pencil"></i></a>
                                </td>
                            @endcan
                            @can('delete-province')
                                <td>
                                    <form action="{{route('province.destroy',['province' => $item->id])}}"
                                          method="POST">
                                        @csrf
                                        @method('delete')
            ...
            <ul class="list-item-small d-block d-lg-none">
                <li class="item-list">
                    ...
                    <ul style="display: none">
                        <li>
                            <span>@lang('text.country_single')</span>
                            <span>{{$item->country_id == 2 ? $data['country'][0][app()->getLocale()] : $data['country'][1][app()->getLocale()]}}</span>
                        </li>
                        @can('edit-province')
                            <li>
